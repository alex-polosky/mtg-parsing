...
class _base:
    def __init__(self, data, readonly=True):
        ...
        for key, annotation in get_type_hints(self.__class__).items():
            annotation: GenericAlias | type
            if key in data:
                if annotation.__name__ == 'list':
                    value = []
                    for each in data[key]:
                        try:
                            value.append(annotation.__args__[0](each))
                        except:
                            value.append(each)
                    ...
                    if annotation == date:
                        value = date.fromisoformat(data[key])
                    else:
                        value = annotation(data[key])
